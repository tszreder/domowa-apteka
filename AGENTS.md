...
- Remote is `tszreder/domowa-apteka` (private). Commits and branches share one Conventional-Commits-style `<type>` vocabulary: `feat` / `fix` / `chore` / `refactor` / `docs`, chosen from the change's nature — `feat` for new user-visible behavior, `fix` for defect corrections, `chore` for maintenance (docs, archiving, dependency/tooling upkeep), `refactor` for restructuring without behavior change, `docs` for documentation-only edits outside `context/**`. Commit subjects are `<type>(<change-id>): <subject>`. Branches spell the type out — `feature/`, `fix/`, `chore/`, `refactor/`, `docs/` — followed by the change-id, e.g. `feature/oauth-login`, `fix/household-accounts-and-invites-review-fixes`. In practice almost everything is `feature` / `fix` / `chore`; reach for `refactor` / `docs` only when neither of those fits. `/10x-new` creates the branch automatically using this convention; `/10x-implement`'s phase-end commits use the same `<type>` set.
