...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "data trapped somewhere + workflow friction"
    - topic: "insight"
      decision: "active-substance matching is the unlock; barcode/photo capture removes the logging friction that kills manual lists"
    - topic: "primary persona scope"
      decision: "any adult household member, symmetrically (no single household-manager role)"
    - topic: "auth strategy"
      decision: "login-based accounts, grouped into a household"
    - topic: "role model"
      decision: "v1 ships adult-only accounts, symmetric; child role (view-assignment-only, no login) deferred to a later version"
    - topic: "MVP flow scope"
      decision: "scoped down from barcode/photo capture to manual name entry + active-substance lookup + shared list, to fit a tight timeline; scanning deferred to v2"
    - topic: "household onboarding scope"
      decision: "kept in v1 — adults can create/join a household and invite other adults; only child sub-accounts are deferred"
    - topic: "multi-substance products & duplicate model"
      decision: "a single pharmaceutical may contain multiple active substances; the list uses a two-tier duplicate model — identical active-substance set = full duplicate (zamiennik), overlapping-but-not-identical set = partial duplicate, flagged distinctly (Model A)"
    - topic: "data source & cleaning approach"
      decision: "authoritative source is the official Polish national medicinal-products registry (public, Health-Ministry-maintained, daily-refreshed); some source fields are messy/free-text and need normalization; a cheap LLM is a candidate for non-user-facing cleanup only. Kept the existing 'never guesses / verified sources' guardrail: substance identity must always trace to the source record; cleanup may only normalize presentation. URL/XML/batch-ingestion/LLM routed to Forward: tech-stack, not the PRD body."
  frs_drafted: 5
---
...
- Timeline-cost ack: present (1-week MVP, under the 3-week soft-gate threshold — no acknowledgment block needed)
- Non-Goals: present (3 entries)

## Forward: tech-stack

Not part of the PRD — routed to `/10x-tech-stack-selector`. Captures the concrete data-source integration the product depends on.

- **Data source**: the official Polish national medicinal-products registry (Rejestr Produktów Leczniczych / RPL), published by the Health Ministry as a public XML report and refreshed daily.
  - Feed: `https://rejestry.ezdrowie.gov.pl/api/rpl/medicinal-products/public-pl-report/6.0.0/overall.xml`
- **Stack driver 1 — scheduled batch ingestion**: the app depends on periodically ingesting and parsing this external XML feed (daily cadence, to match the registry), not a live per-request lookup.
- **Stack driver 2 — normalization of messy source data**: some source fields are free-text and inconsistent (suffixes/prefixes to strip, combined values to split, formatting to normalize) before they are usable for autocomplete and active-substance matching.
  - Candidate approach (user's words, not a commitment): "we might need a simple, cheap LLM" for this normalization step.
  - **Hard boundary carried from the product guardrail**: normalization may only clean *presentation*; a product's active-substance *identity* must always trace to its source record. No LLM-invented or inferred substance identities.
