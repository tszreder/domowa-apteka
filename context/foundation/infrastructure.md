...
| Daily ingestion cron skips runs silently when the prior execution is still `Active`, and fires ±minutes | Unknown unknowns (confirmed in docs) | M | H | App-side last-successful-run timestamp surfaced in admin. Never treat "cron fired" as freshness. |
